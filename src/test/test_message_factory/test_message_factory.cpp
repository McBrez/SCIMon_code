...
TEST_CASE("Test the encoding and decoding of messages") {
  ...
  std::shared_ptr<Device> dummySource(new DummyDevice());
  std::shared_ptr<Device> dummyDestination(new DummyDevice());

  SECTION("Handshake Message") {
    list std::shared_ptr < StatusPayload >> statusPayloads;
    statusPayloads.emplace_back std::shared_ptr<StatusPayload>(
    ...
    std::shared_ptr<DeviceMessage> handshakeMessage(
    ...
    std::shared_ptr<DeviceMessage> decodedMessage =
    ...
    list std::shared_ptr < StatusPayload >> origPayloads =
        messageOrig->getPayload();
    list std::shared_ptr < StatusPayload >> resultPayloads =
        result->getPayload();
